...
int bitXor(int x, int y) {
  ...
  int min_y=~y;
  int and_xy=x&y;
  int and_min_xy=min_x&min_y;
  return (~and_min_xy)&(~and_xy);
}
/* 
...
int logicalNeg(int x) {
  int signX=(x>>31)&0x01;
  int negX=(~x)+1;
  int sign_negX=(negX>>31)&0x01;
  int result=signX|sign_negX;
  result=(~result)&0x01;
  return result;
